/* Control de TRIAC con potenciometro
 * Autor: ELECTRONOOBS
 * Suscríbete: http://www.youtube.com/c/ELECTRONOOBS
 * Tutorial: http://www.ELECTRONOOBS.com/eng_circuitos_tut32.php
 * Gracias
 */

// Variables globales
int detectado = 0; // Indica si se ha detectado un cruce por cero
int valor = 0; // Valor del potenciometro
int last_CH1_state = 0; // Ultimo estado del pin D8
...
void loop() {
  ...
  valor = map(analogRead(A2), 0, 1024, 7200, 10); // Mapeo Orden de Disparo

  // Si se ha detectado un cruce por cero
  if (detectado) {
    // Esperar un tiempo proporcional al valor del potenciometro
    delayMicroseconds(valor); // Orden de disparo

    // Activar el TRIAC (pin D3)
    digitalWrite(3, HIGH);

    // Esperar 100 microsegundos
    delayMicroseconds(100);

    // Desactivar el TRIAC (pin D3)
    digitalWrite(3, LOW);

    // Reiniciar la variable detectado
    detectado = 0;
  }
}
