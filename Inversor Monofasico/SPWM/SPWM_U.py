...
-------------------------------------------------------------------
Modulación UNIPOLAR con portadora triangular en [0,1].
- Referencias: ref_A = 0.5*(1 + ma*seno), ref_B = 0.5*(1 - ma*seno)
- D9 y D10 se activan cuando su referencia supera la portadora.
- Salida: Vout = D9 - D10  (+Vdc, 0, -Vdc)

Dependencias: numpy, matplotlib, tkinter
Autor: Prof. Alexander Bueno (modificado)
Fecha: 08/2026
"""
...
def evaluar_referencia(tipo, theta, params):
    ...
    Retorna:
        array: valores de referencia entre -1 y 1
    """
    if tipo == "SPWM":
        ref = np.sin(theta)
    elif tipo == "THIPWM":
...
def generar_referencia(tipo, f_ref, mf, params):
    """
    Genera la tabla de referencia normalizada (sin multiplicar por ma)
    para un periodo de la portadora.
    """
...
# =============================================================================
#  MODULO DE CALCULO DEL TEMPORIZADOR Y GENERACION DEL CODIGO ARDUINO
# =============================================================================

def calcular_parametros_timer(f_ref, mf, dead_time_us, ma, ref_norm):
    """
    Calcula la configuración del Timer1 (prescaler, ICR1) y las tablas
    OCR1A y OCR1B para modulación unipolar.
    ref_norm está en [-1, 1].
    """
    F_CLK = 16000000
    ...
    # ---- Cálculo de OCR para modulación unipolar ----
    # ref_norm en [-1,1], ma en [0,1]
    d_A = 0.5 * (1 + ma * ref_norm)
    d_B = 0.5 * (1 - ma * ref_norm)

    OCR1A_raw = d_A * best_ICR
    OCR1B_raw = d_B * best_ICR

    # Aplicar tiempo muerto restando a ambas cuando están activas
...
def generar_archivo_ino(f_ref, ma, mf, dead_time_us, tipo, params,
    ...
    ambos canales como no inversores (modulación unipolar).
    """
    ruta = "SPWM.ino"
    ...
    with open(ruta, "w") as f:
        ...
        f.write("//  Modo: Phase Correct PWM con portadora triangular [0,1]\n")
        f.write("//  MODULACIÓN UNIPOLAR\n")
        f.write("// ================================================================\n")
        ...
        f.write("void setup() {\n")
        f.write("  pinMode(9, OUTPUT);   // Rama A del puente H\n")
        f.write("  pinMode(10, OUTPUT);  // Rama B del puente H\n\n")
        ...
        f.write("  // OC1B: no inversor (COM1B1=1, COM1B0=0)  <-- MODULACIÓN UNIPOLAR\n")
...
def simular_senales(f_ref, ma, mf, dead_time_us, ref_norm, pts_por_periodo=200):
    """
    Simula las señales PWM (D9, D10) y la tensión de salida (D9-D10)
    para dos ciclos completos de la referencia.
    Portadora triangular en [0,1], modulación unipolar.
    ref_norm está en [-1,1].
    """
    f_c = mf * f_ref
    T_c = 1.0 / f_c
    ...
    # Portadora triangular en [0,1] (pico en fase=0.5, valle en 0 y 1)
    fase = (t % T_c) / T_c
    portadora = 2 * np.minimum(fase, 1 - fase)   # 0..1

    # Referencia repetida
    ref_norm_repetida = np.repeat(np.tile(ref_norm, 2), pts_por_periodo)

    # Referencias para cada rama
    ref_A = 0.5 * (1 + ma * ref_norm_repetida)
    ref_B = 0.5 * (1 - ma * ref_norm_repetida)

    # Comparación con la portadora
    outA = (ref_A > portadora).astype(float)
    outB = (ref_B > portadora).astype(float)

    # Tensión de salida (normalizada)
    diff = outA - outB   # +1, 0, -1

    return t, outA, outB, diff
...
def graficar_simulacion(f_ref, ma, mf, dead_time_us, ref_norm, tipo, params):
    """
    Muestra tres subplots en vertical:
      1) Referencias A y B + portadora triangular
      2) PWM de D9 y D10 desplazadas verticalmente
      3) Tensión de salida (D9 - D10)
    Retorna (RMS, THD).
    """
    # Simulación
    t, outA, outB, diff = simular_senales(f_ref, ma, mf, dead_time_us, ref_norm, pts_por_periodo=200)
    try:
    ...
    ref_high = evaluar_referencia(tipo, theta_high, params)   # en [-1,1]
    ref_A_high = 0.5 * (1 + ma * ref_high)
    ref_B_high = 0.5 * (1 - ma * ref_high)   # Referencia para D10
    # Portadora triangular en [0,1]
    fase_high = (t_high % T_c) / T_c
    portadora_high = 2 * np.minimum(fase_high, 1 - fase_high)

    # Crear figura con 3 subplots verticales
    fig, axs = plt.subplots(3, 1, figsize=(14, 12))
    fig.suptitle(
        f'PWM con portadora triangular [0,1] - Modulación UNIPOLAR\n'
        f'f_ref = {f_ref:.1f} Hz, ma = {ma:.2f}, mf = {mf}, Tmuerto = {dead_time_us:.1f} us\n'
        f'Tipo: {tipo}  |  RMS = {V_rms:.4f}  |  THD = {thd:.2f}%',
        fontsize=14
    )

    # ---- Subplot 1: Referencias A y B + portadora ----
    axs[0].plot(t_high, ref_A_high, 'b', lw=1.5, label='Referencia A (D9)')
    axs[0].plot(t_high, ref_B_high, 'orange', lw=1.5, label='Referencia B (D10)')
    axs[0].plot(t_high, portadora_high, 'r', lw=0.6, label='Portadora')
    axs[0].axhline(y=0.5, color='gray', linestyle='--', linewidth=0.5)
    axs[0].set_ylabel('Amplitud')
    axs[0].grid(True)
    axs[0].legend()
    axs[0].set_title('Referencias A y B + Portadora (alta resolución)')
    axs[0].set_xlim(t_high[0], t_high[-1])

    # ---- Subplot 2: PWM D9 y D10 desplazadas ----
    offset = 1.0
    ...
    axs[1].grid(True)
    axs[1].legend()
    axs[1].set_title('Señales PWM en D9 y D10 (desplazadas verticalmente)')
...
class SPWMApp:
    ...
    def __init__(self, root):
        self.root = root
        root.title("Generador PWM para Arduino - Puente H (UNIPOLAR)")
    ...
    def _obtener_referencia(self):
        """Construye el diccionario de parámetros y genera la referencia normalizada."""
        ...
        return generar_referencia(tipo, f_ref, mf, params)   # en [-1,1]
    ...
    def _graficar(self):
        ...
        try:
            ...
            ref_norm = self._obtener_referencia()
            params = {}
            if tipo == "THIPWM":
                params['a3'] = float(self.param_a3.get())
            elif tipo == "MPWM":
            ...
            V_rms, thd = graficar_simulacion(f_ref, ma, mf, dead, ref_norm, tipo, params)
    ...
    def _generar_codigo(self):
        ...
        try:
            ...
            t, _, _, diff = simular_senales(f_ref, ma, mf, dead, ref_norm, pts_por_periodo=200)
            V_rms, _, thd = calcular_rms_thd(t, diff, f_ref)

            # ---- Calcular parámetros del timer (Phase Correct) ----
            prescaler, ICR, f_c_actual, dead_ticks, OCR1A, OCR1B, N = \
                calcular_parametros_timer(f_ref, mf, dead, ma, ref_norm)
